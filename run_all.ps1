...
#    Run-Train -tag 'paper_multi_weighted_cka_forget' -seed $seed -arch 'hrl' -llMode 'multi' -experienceSharing -shareMode 'weighted_cka' --enable_forgetting --forget_rate 5e-5 --forget_floor 0.25 --retention_decay 0.9995
    ...
    #
    # +Tutee + ES (CFA/wCKA)
    Run-Train -tag 'tutee_weighted_cka' -seed $seed -arch 'hrl' -llMode 'multi' -useTutee -experienceSharing -tuteeLLPolicy 'learned' -shareMode 'weighted_cka' --obs_include_teach_boost --enable_forgetting --forget_rate 2e-5 --forget_floor 0.30 --retention_decay 0.9995  --retention_from_tutee 0.12
    #
    # Control A: HL can choose tutee, but tutee LL is random (ALL actions) and we disable its LL training
#    Run-Train -tag 'tutee_controlA_randLL_all' -seed $seed -arch 'hrl' -llMode 'multi' -useTutee -experienceSharing -tuteeLLPolicy 'random_all' -shareMode 'weighted_cka' -tuteeDisableLLTraining --obs_include_teach_boost --enable_forgetting --forget_rate 5e-5 --forget_floor 0.25 --retention_decay 0.9995  --retention_from_tutee 0.10
    #
    # Control A (variant): random only among "ready"/allowed tutee actions
#    Run-Train -tag 'tutee_controlA_randLL_ready' -seed $seed -arch 'hrl' -llMode 'multi' -useTutee -experienceSharing -tuteeLLPolicy 'random_allowed' -shareMode 'weighted_cka' -tuteeDisableLLTraining --obs_include_teach_boost --enable_forgetting --forget_rate 5e-5 --forget_floor 0.25 --retention_decay 0.9995  --retention_from_tutee 0.10
}
